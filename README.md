...
| qsub_valid           | Whether the `qsub_options` were accepted by the scheduler's syntax check (`qsub -w p`) at discovery time: `True` (valid) or `False`. A `False` row is automatically reported as **FAILED** by the pipeline and is **not** submitted to the scheduler (see [Step 3](#step-3---review-the-results)). |
...
A test is automatically marked **FAILED without being run** if its `qsub_valid` value in the input CSV was `False` (its qsub options were rejected by `qsub -w p` during discovery, and would be rejected again at job submission). These rows are handled locally by the pipeline (never submitted to the scheduler) and show `exit_code` of `1`, `tests_failed` of `1`, and a `job_number` of `NA` in the report. Their `results.txt` contains an error message explaining why the test was not run.
...
NOTE: For a test that was skipped due to invalid qsub options (`qsub_valid=False` in the input CSV), the working directory only contains `results.txt` (with the error message) and `test_metrics.csv`. There is no copied `test` directory and no `log.txt`, because `test.qsub` was never run.
...
- **OPTION 1:** In the directory where the nextflow script ran, there should be a hidden file called `.nextflow.log`.  This will contain information about the nextflow pipeline and actions taken for each process.  In this file, search for the keyword "Error", "Failed", or "Terminated" to find which process (or modules) failed. Sometimes additional information is shown in these logs that may indicate what went wrong.
  ...
  **Bad qsub options.** NOTE: Most invalid qsub options are now detected during discovery by `find_qsub.py` (the `qsub_valid` column) and reported directly as **FAILED** — the test is not submitted, so you will see it in the report CSV rather than as a submission failure here (see [Step 3](#step-3---review-the-results)). A submission failure like the one below can still occur for options that pass the `qsub -w p` syntax check but are rejected at actual submission. Below is an example exerpt from a `.nextflow.log` file of a "Failed" job submission for module R/4.3.1 test.
